    ...
    return (
        <div className={`w-full h-[450px] md:h-[550px] flex items-center justify-center perspective-1000 overflow-hidden relative rounded-3xl border shadow-2xl mb-8 group transition-all duration-500 ${isFixed ? 'bg-[#0a1820] border-emerald-500/20 shadow-emerald-500/10' : 'bg-[#1a0a0a] border-red-500/20 shadow-red-500/10'}`}>
            ...
            <div className={`relative w-[200px] h-[400px] transform-style-3d animate-float cursor-pointer transition-transform duration-700 ${isFixed ? 'hover:rotate-x-6 hover:rotate-y-6' : 'hover:rotate-x-12 hover:rotate-y-12'}`}>
                ...
                {/* KATMAN 2: ANAKART */}
                <div className={`absolute inset-0 backdrop-blur-md rounded-[3rem] border shadow-xl transform transition-transform duration-500 flex flex-col items-center justify-center gap-3 p-4 ${isFixed ? 'bg-black/80 border-emerald-500/30 translate-z-0 group-hover:translate-z-0' : 'bg-black/60 border-red-500/30 translate-z-0 group-hover:translate-z-0'}`}>
                ...
                {/* KATMAN 3: EKRAN */}
                <div className={`absolute inset-0 rounded-[3rem] border-2 shadow-2xl transform transition-transform duration-500 overflow-hidden flex flex-col items-center justify-center ${isFixed ? 'bg-gradient-to-tr from-slate-900 to-emerald-900/20 border-emerald-500/20 translate-z-[10px] group-hover:translate-z-[20px]' : 'bg-gradient-to-tr from-slate-950 to-red-950/50 border-red-500/20 translate-z-[40px] group-hover:translate-z-[80px]'}`}>
                    {!isFixed && (<svg className="absolute inset-0 w-full h-full pointer-events-none opacity-40" viewBox="0 0 200 400"><path d="M0,0 L200,400 M200,0 L0,400 M100,0 L100,400 M0,200 L200,200" stroke="rgba(255,255,255,0.2)" strokeWidth="1" fill="none"/><circle cx="100" cy="200" r="50" stroke="rgba(255,255,255,0.3)" strokeWidth="1" fill="none"/></svg>)}
                    ...
                    <div className="relative z-10 text-center">
                        ...
                        <div className={`text-xs font-mono mt-2 px-3 py-1 rounded-full font-bold uppercase ${isFixed ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400 animate-pulse'}`}>{isFixed ? (statusLower.includes('teslim') ? 'TESLİM EDİLDİ' : 'SİSTEM HAZIR') : status.toUpperCase()}</div>
                    </div>
                </div>
            </div>
            <div className={`absolute bottom-6 text-center w-full pointer-events-none text-xs font-bold tracking-[0.2em] animate-pulse ${isFixed ? 'text-emerald-500/50' : 'text-red-500/50'}`}>{isFixed ? 'ASSEMBLED // READY' : 'EXPLODED VIEW // DIAGNOSTIC MODE'}</div>
        </div>
    );
};
...
function CihazSorgulaContent() {
  ...
  // Veri Ayrıştırma Yardımcısı
  const parseData = (data: any) => { if (!data) return []; if (Array.isArray(data)) return data; try { return JSON.parse(data); } catch { return []; } };
  
  // Durum Eşleştirme Yardımcısı (Türkçe karakter duyarlı)
  const normalize = (str: string) => str.toLocaleLowerCase('tr').trim();
  const checkStatus = (current: string, target: string) => normalize(current).includes(normalize(target));
  ...
  const performSearch = async (val: string) => {
    if (!val) return; setLoading(true); setResult(null); setError(""); 
    const rawInput = val.trim().toUpperCase().replace(/\s/g, ''); const numericPart = rawInput.replace(/\D/g, '');
    try {
    ...
    } catch (err: any) { console.error(err); setError("Bağlantı hatası."); } finally { setLoading(false); }
  };
  ...
  const handleClientApproval = async (decision: 'approved' | 'rejected') => {
      ...
      // Onaylanırsa durum 'İşlemde' olsun
  ...
  const handleBuyUpsell = async (item: any) => {
      if(!confirm(`${item.name} eklemek istiyor musunuz?`)) return; setLoading(true);
      const newPrice = Number(result.price) + Number(item.price);
      
      // Listeleri güncelle: Önerilenlerden çıkar, Satılanlara ekle
  ...
  // Timeline mantığı (normalize fonksiyonu kullanıldı)
  const currentStep = result ? (
    checkStatus(result.status, "Teslim") ? 6 :
    checkStatus(result.status, "Hazır") ? 5 :
    checkStatus(result.status, "Onay") ? 4 :
    checkStatus(result.status, "Parça") ? 3 :
    checkStatus(result.status, "İşlem") ? 2 : 1
  ) : 0;
  ...
  // Upsell'leri Ayır
  const services = recUpsells.filter((i:any) => i.type === 'service' || SERVICE_CATEGORIES.includes(i.category));
  const products = recUpsells.filter((i:any) => i.type !== 'service' && !SERVICE_CATEGORIES.includes(i.category));

  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans relative overflow-x-hidden selection:bg-cyan-500/30">
      <div className="fixed inset-0 pointer-events-none z-0"><div className="absolute inset-0 bg-[linear-gradient(to_right,#80808008_1px,transparent_1px),linear-gradient(to_bottom,#80808008_1px,transparent_1px)] bg-[size:48px_48px]"></div><div className="absolute top-[-10%] left-[-10%] w-[600px] h-[600px] bg-cyan-600/10 rounded-full blur-[120px]"></div><div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-purple-600/10 rounded-full blur-[120px]"></div></div>
                ...
                <div className="lg:col-span-7 space-y-6">
                    ...
                    <div className="bg-[#0F1623]/80 backdrop-blur-md border border-slate-800 rounded-3xl p-8 shadow-xl relative overflow-hidden">
                        <div className="relative z-10 flex justify-between items-start">
                            <div><span className="px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-black uppercase tracking-widest mb-2 inline-block">{result.category || "SERVİS"}</span><h2 className="text-3xl font-black text-white mb-1">{result.device_name || result.device}</h2><div className="text-slate-400 text-sm flex items-center gap-2"><User size={14}/> {maskName(result.customer_email || result.customer || "")}</div></div>
                            <div className="text-right"><div className="text-4xl font-black text-white">{Number(result.price).toLocaleString('tr-TR')} ₺</div><div className="text-xs text-slate-500 font-bold mt-1">TOPLAM TUTAR</div></div>
                        </div>
                    </div>

                    {result.approval_status === 'pending' && (<div className="bg-yellow-950/20 border border-yellow-500/30 rounded-2xl p-6 relative overflow-hidden group"><div className="absolute inset-0 bg-yellow-500/5 group-hover:bg-yellow-500/10 transition-colors"></div><div className="relative z-10"><div className="flex items-center gap-3 mb-3 text-yellow-400"><AlertTriangle size={24} className="animate-bounce"/><h3 className="text-lg font-bold">Ekstra İşlem Onayı Gerekiyor</h3></div><p className="text-slate-300 text-sm mb-4 bg-black/20 p-3 rounded-lg border border-yellow-500/10">{result.approval_desc}</p><div className="flex items-center justify-between"><div className="text-xl font-black text-white">+{result.approval_amount} ₺ <span className="text-xs font-normal text-slate-400">Ek Ücret</span></div><div className="flex gap-2"><button onClick={() => handleClientApproval('rejected')} className="px-4 py-2 rounded-lg border border-slate-600 text-slate-400 hover:text-white hover:bg-slate-800 text-sm font-bold">Reddet</button><button onClick={() => handleClientApproval('approved')} className="px-6 py-2 rounded-lg bg-yellow-500 hover:bg-yellow-400 text-black font-bold text-sm shadow-lg shadow-yellow-500/20">Onayla</button></div></div></div></div>)}
                    ...
                    {(services.length > 0 || products.length > 0) && (
                        <div className="bg-gradient-to-br from-pink-900/10 to-purple-900/10 border border-pink-500/20 rounded-2xl p-6">
                            <h3 className="text-xs font-bold text-pink-400 uppercase mb-4 flex items-center gap-2"><ShoppingBag size={14}/> Sizin İçin Önerilenler</h3>
                            
                            {services.length > 0 && (
                                <div className="mb-4">
                                    <h4 className="text-[10px] font-bold text-purple-400 mb-2 tracking-widest">EK HİZMETLER</h4>
                            ...
                            {products.length > 0 && (
                                <div>
                                    <h4 className="text-[10px] font-bold text-cyan-400 mb-2 tracking-widest">AKSESUARLAR</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="bg-[#151921] border border-slate-800 rounded-2xl p-6">
                        ...
                        <div className="bg-[#0b0e14] rounded-xl p-4 border border-slate-700"><p className="text-sm text-slate-300 whitespace-pre-line leading-relaxed">{result.technician_note || "Teknisyen notu henüz girilmemiş."}</p></div>
...
{result.status === 'Teslim Edildi' && result.warranty_end_date && (
    <div className="mt-6 relative overflow-hidden rounded-3xl border border-emerald-500/30 bg-[#0f1219] shadow-[0_0_40px_rgba(16,185,129,0.1)] group">
        ...
        {selectedImage && (<div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in" onClick={() => setSelectedImage(null)}><img src={selectedImage} className="max-w-full max-h-[90vh] rounded-xl shadow-2xl border border-white/10"/><button className="absolute top-6 right-6 text-white bg-black/50 p-2 rounded-full"><X size={24}/></button></div>)}
      </div>
    </div>
  );
}