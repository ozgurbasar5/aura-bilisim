...
export default function YonetimPaneli() {
  ...
  useEffect(() => {
    ...
    storeProducts.forEach((prod: any) => {
        // Satılanlar (Ciroya Dahil Et)
        // Not: Gerçek senaryoda satış tarihine bakılmalı, şimdilik statü bazlı.
        ...
        // Operasyonel (Sipariş Süreci) -> Bunlar "Sanal" statüler, projende yoksa varsayılan atarız.
        // Eğer mağaza modülünde "Kargolandı" gibi statüler eklediysen burayı ona göre güncelle.
        if (prod.status === "Paketlemede") mPacking++; 
        if (prod.status === "Kargoda") mShipped++;

        // Stoktaki Ürünler (Satışta)
        if (prod.status === "Satışta") {
            mStockVal += Number(prod.price) || 0;
            if (activeStockList.length < 5) { // Sadece ilk 5 ürünü listeye al
