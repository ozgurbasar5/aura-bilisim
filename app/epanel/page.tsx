...
import { 
  ...
  Layers, Filter, Command
} from "lucide-react";
import { supabase } from "@/app/lib/supabase"; 

// --- PATRON MODU BİLEŞENLERİ ---

// 1. Neon Glass Kart (Hatasız Tema Yapısı)
const NeonCard = ({ children, className = "", title, icon: Icon, action, theme = "cyan" }: any) => {
    const themes: any = {
...
// 2. Finansal Bar (Gelir/Gider İçin)
...
const TechBar = ({ width, color }: { width: string, color: string }) => (
  <div className="h-1.5 w-full bg-[#1e293b] rounded-full overflow-hidden mt-3 relative">
...
export default function EPanelDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    targetRevenue: 150000 // Varsayılan aylık hedef (Ayarlanabilir)
  });
  ...
  useEffect(() => {
    async function fetchData() {
        ...
        if (jobs) {
            jobs.forEach((job: any) => {
                ...
                if (!status.includes("teslim") && !status.includes("hazır") && !status.includes("iptal") && !status.includes("iade") && !status.includes("red")) {
                    totalPending += price;
                    if (status.includes("bekliyor") || status.includes("onay")) sWaiting++; else sProcessing++;
                }
        ...
        let topCat = maxCatVal === catPhone ? "Cep Telefonu" : maxCatVal === catRobot ? "Robot Süpürge" : maxCatVal === catPc ? "Bilgisayar" : "Diğer";
        ...
        setStats({
            ...
            targetRevenue: 150000 // Varsayılan hedef
        });
        setLoading(false);
    }
    fetchData();
  ...
  if (loading) return (
      <div className="h-full flex flex-col items-center justify-center gap-6 bg-[#020617]">
          ...
          <div className="font-mono text-cyan-500 text-xs tracking-[0.3em] animate-pulse">İŞ ZEKASI YÜKLENİYOR...</div>
      </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
        ...
        {/* --- BÖLÜM 1: FİNANSAL KPI KARTLARI (Patron Odaklı) --- */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard 
                title="AYLIK CİRO" 
                value={`${stats.monthlyRevenue.toLocaleString('tr-TR')} ₺`}
                subValue={`${stats.serviceCount + stats.storeSoldThisMonth} İşlem`}
                icon={DollarSign}
                color="cyan"
                trend={`%${stats.profitMargin} Kâr`}
            />
            <StatCard 
                title="NET KÂR" 
                value={`${stats.monthlyProfit.toLocaleString('tr-TR')} ₺`}
                subValue={`Gider: ${stats.monthlyCost.toLocaleString('tr-TR')} ₺`}
                icon={Wallet}
                color="emerald"
            />
            <StatCard 
                title="BEKLEYEN ALACAK" 
                value={`${stats.bekleyenAlacak.toLocaleString('tr-TR')} ₺`}
                subValue={`${stats.waiting + stats.processing} Aktif Dosya`}
                icon={Clock}
                color="yellow"
            />
            <StatCard 
        ...
        {/* --- BÖLÜM 2: ANA PANELLER --- */}
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            
            {/* SOL PANEL: ATÖLYE HAREKETLERİ */}
            <div className="xl:col-span-2 space-y-6">
                
                {/* Atölye Durum Kartları */}
                <NeonCard title="ATÖLYE HAREKETLİLİĞİ" icon={Wrench} className="p-6" theme="blue">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        {/* Bekleyen */}
                        <div className="bg-[#0f1219] p-5 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-amber-500/40 transition-colors shadow-lg">
                            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Clock size={40} className="text-amber-500"/></div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">BEKLEYEN</h4>
                            <div className="text-3xl font-black text-white mb-2">{stats.waiting}</div>
                            <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                                <div className="h-full bg-amber-500" style={{ width: stats.activeServiceTotal > 0 ? `${(stats.waiting/stats.activeServiceTotal)*100}%` : '0%' }}></div>
                            </div>
                        </div>
                        {/* İşlemde */}
                        <div className="bg-[#0f1219] p-5 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-blue-500/40 transition-colors shadow-lg">
                            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Wrench size={40} className="text-blue-500"/></div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">İŞLEM GÖREN</h4>
                            <div className="text-3xl font-black text-white mb-2">{stats.processing}</div>
                            <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                                <div className="h-full bg-blue-500" style={{ width: stats.activeServiceTotal > 0 ? `${(stats.processing/stats.activeServiceTotal)*100}%` : '0%' }}></div>
                            </div>
                        </div>
                        {/* Hazır */}
                        <div className="bg-[#0f1219] p-5 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-emerald-500/40 transition-colors shadow-lg">
                            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><CheckCircle2 size={40} className="text-emerald-500"/></div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">HAZIR & TESLİM</h4>
                            <div className="text-3xl font-black text-white mb-2">{stats.ready}</div>
                            <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500" style={{ width: stats.activeServiceTotal > 0 ? `${(stats.ready/stats.activeServiceTotal)*100}%` : '0%' }}></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            { label: "Telefon", val: stats.phone, ic: Smartphone, c: "text-purple-400 bg-purple-500/10 border-purple-500/20" },
                            { label: "Robot", val: stats.robot, ic: Zap, c: "text-amber-400 bg-amber-500/10 border-amber-500/20" },
                            { label: "Bilgisayar", val: stats.pc, ic: Laptop, c: "text-blue-400 bg-blue-500/10 border-blue-500/20" },
                            { label: "Diğer", val: stats.other, ic: Box, c: "text-slate-400 bg-slate-500/10 border-slate-500/20" },
                        ].map((item, i) => (
                            <div key={i} className={`flex items-center gap-3 p-3 bg-slate-900/50 rounded-xl border hover:bg-slate-800 transition-colors ${item.c}`}>
                                <div className={`p-2 rounded-lg ${item.c.split(' ').slice(1).join(' ')} shadow-inner`}><item.ic size={18}/></div>
                                <div>
                                    <div className="text-lg font-bold text-white leading-none">{item.val}</div>
                                    <div className="text-[10px] text-slate-500 uppercase font-bold mt-1">{item.label}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* YENİ PATRON PANELLERİ (TEKNİK YERİNE) */}
            ...
            {/* SAĞ PANEL: HIZLI ERİŞİM VE İZLEME */}
            <div className="space-y-6">
                
                {/* Hızlı Erişim Kokpiti */}
                ...
                <NeonCard title="DEPO DURUMU" icon={Package} className="p-5" theme="emerald">
                    <div className="space-y-3 mb-6">
                        <div className="flex justify-between items-center text-xs p-3 bg-[#0f1219] rounded-xl border border-white/5 hover:border-purple-500/30 transition-colors shadow-lg group">
