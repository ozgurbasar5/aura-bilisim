...
export default function Navbar() {
  ...
  // ARAMA FONKSİYONU (Sadece Yönlendirme Yapar)
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault(); 
    if (searchQuery.trim().length > 0) {
      router.push(`/cihaz-sorgula?q=${encodeURIComponent(searchQuery)}`);
      setMenuAcik(false); // Mobil menü açıksa kapat
    }
  };

  // E-Panel veya Login sayfalarında bu Navbar'ı gizle
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out border-b ${
      scrolled 
        ...
        <div className="container mx-auto px-6 h-full flex items-center justify-between transition-all duration-500">
          ...
          {/* --- SAĞ TARAF İKONLAR & ARAMA --- */}
          <div className="flex items-center gap-3 shrink-0">
              
              {/* CİHAZ SORGULA INPUT */}
              <form 
                onSubmit={handleSearch}
                className="hidden lg:flex items-center gap-2 border border-slate-700/50 bg-[#0f172a] hover:bg-slate-800 focus-within:ring-1 focus-within:ring-cyan-500/50 text-slate-200 px-4 py-2.5 rounded-xl font-bold text-xs transition-all group w-64"
              >
                  ...
                  <input 
                    ...
                    placeholder="CİHAZ SORGULA" 
                    className="bg-transparent border-none outline-none w-full text-slate-200 placeholder:text-slate-500 font-bold text-xs"
                  />
              </form>
